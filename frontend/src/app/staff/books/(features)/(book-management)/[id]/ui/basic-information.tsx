...
export default function BasicInformation({ book }: { book: BookDetail }) {
  ...
  const resetForm = () => {
    setFormData({
      isbn: book.isbn,
      title: book.title,
      publisherId: publisherOptions.find((item) => item.name === book.publisher)?.id || '',
      pageCount: String(book.pageCount ?? ''),
      languageId: languageOptions.find((item) => item.name === book.language)?.id || '',
      genreIds: genreOptions.filter((item) => book.genres.includes(item.name)).map((item) => item.id),
      authorIds: authorOptions.filter((item) => book.authors.includes(item.name)).map((item) => item.id),
      description: book.description ?? '',
    });
  };
    ...
    <section className="rounded-3xl bg-white p-10 shadow-sm border border-[#dbe5dd]">
      <div className="mb-8 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {isEditing ? (
            <>
              <button type="button" onClick={() => { resetForm(); setIsEditing(false); }} disabled={isSaving} className="inline-flex items-center gap-2 rounded-xl border border-[#b8c5bd] bg-white px-5 py-2.5 text-sm font-bold text-[#58615b] transition-colors hover:bg-[#f4f7f5] disabled:cursor-not-allowed disabled:opacity-60">Cancel</button>
              <button type="button" onClick={handleSave} disabled={isSaving} className="inline-flex items-center gap-2 rounded-xl bg-[#3f6754] px-5 py-2.5 text-sm font-bold text-[#e6ffef] transition-colors hover:bg-[#335b48] disabled:cursor-not-allowed disabled:opacity-60">{isSaving ? 'Saving...' : 'Save'}</button>
            </>
          ) : (
            <button type="button" onClick={() => { resetForm(); setIsEditing(true); }} className="inline-flex items-center gap-2 rounded-xl bg-[#3f6754] px-5 py-2.5 text-sm font-bold text-[#e6ffef] transition-colors hover:bg-[#335b48]">Edit</button>
          )}
        </div>
      </div>
      {isEditing ? (
