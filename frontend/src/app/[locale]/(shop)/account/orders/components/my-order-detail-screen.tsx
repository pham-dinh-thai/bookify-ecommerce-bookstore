...
export default function MyOrderDetailScreen({ id }: MyOrderDetailScreenProps) {
  ...
  const cancelOrder = async () => {
    ...
    try {
      ...
      toast?.addToast(t('toast.orderCanceled'), 'success');
      await retry();
    } catch (err) {
      toast?.addToast(
  ...
  const retryPayment = async () => {
    ...
    try {
      ...
      toast?.addToast(t('toast.redirectingPayment'), 'success');
              ...
              <div className="flex flex-wrap gap-2">
                ...
                (order.paymentStatus === 'failed' ||
                  ...
                  >
                    ...
                    {retrying ? t('detail.redirecting') : t('detail.payNow')}
                  </button>
                ) : null}
                ...
                {order && isCancellableOrder(order) ? (
                  ...
                  >
                    ...
                    {canceling ? t('canceling') : t('detail.cancelOrder')}
                  </button>
                ) : null}
              </div>
            </header>
    ...
    <div className="w-full max-w-4xl space-y-6">
      <section className="rounded-lg bg-[#eff5ef] p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 border-b border-[#d7e3d8] pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-sm font-bold text-[#58615b]">{t('detail.orderCode')}</p>
            <h2 className="mt-1 text-2xl font-extrabold text-[#2b352f]">
              {order.orderCode}
            </h2>
            <p className="mt-2 text-sm font-medium text-[#58615b]">
              {t('placed', { date: formatDateTime(order.createdAt) })}
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Info label={t('detail.updatedAt')} value={formatDateTime(order.updatedAt)} />
          <Info label={t('detail.totalItems')} value={String(order.totalItems)} />
          <Info
            label={t('detail.payment')}
            value={order.paymentMethod === 'cash_on_delivery' ? t('detail.cashOnDelivery') : 'VNPay'}
          />
          <Info label={t('detail.totalAmount')} value={formatVnd(order.totalAmount)} />
        </div>
      </section>
      ...
      <section className="rounded-lg bg-[#eff5ef] p-5 shadow-sm sm:p-6">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <Info label={t('detail.phoneNumber')} value={order.phoneNumber} />
          <Info label={t('detail.shippingAddress')} value={order.shippingAddress} />
        </div>
      </section>

      <section className="rounded-lg bg-[#eff5ef] p-5 shadow-sm sm:p-6">
        <h3 className="text-xl font-bold text-[#2b352f]">{t('detail.orderedBooks')}</h3>
          ...
          {order.items.length === 0 ? (
            ...
            order.items.map((item) => (
              ...
              >
                ...
                <LineMetric label={t('qty')} value={String(item.quantity)} />
                <LineMetric
                  label={t('unitPrice')}
