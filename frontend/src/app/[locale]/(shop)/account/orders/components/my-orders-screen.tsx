...
type OrderStatusFilterOption = { value: OrderStatusFilter; translationKey: string };

const statusFilterOptions: OrderStatusFilterOption[] = [
  { value: 'pending', translationKey: 'status.pending' },
  { value: 'confirmed', translationKey: 'status.confirmed' },
  { value: 'delivering', translationKey: 'status.delivering' },
  { value: 'delivered', translationKey: 'status.delivered' },
...
export default function MyOrdersScreen() {
  ...
  useEffect(() => {
    ...
    if (payment === 'fail') {
      ...
      toast?.addToast(t('toast.paymentSuccess'), 'success');
    } else if (payment === 'error') {
      toast?.addToast(
        t('toast.paymentError'),
        'error',
      );
    }
        ...
        <div className="min-w-0 flex-1">
          ...
          {loading ? (
            ...
            <div className="w-full max-w-4xl rounded-lg border border-dashed border-[#3f6754]/20 bg-white p-8 text-center">
              ...
              />
              <h2 className="text-2xl font-bold text-[#2b352f]">
                {t('emptyFilter', { status: t(statusFilterOptions
    ...
    <article className="rounded-lg bg-[#eff5ef] p-5 shadow-sm sm:p-6">
        ...
        {previewItems.length === 0 ? (
          ...
          previewItems.map((item) => (
            ...
            >
              ...
              <LineMetric label={t('qty')} value={`x${item.quantity}`} />
              <LineMetric label={t('unitPrice')} value={formatVnd(item.unitPrice)} />
              <LineMetric label={t('lineTotal')} value={formatVnd(item.lineTotal)} />
            </div>
          ))
        )}
      ...
      <div className="mt-5 flex flex-col gap-2 border-t border-[#d7e3d8] pt-5 text-[#2b352f] sm:flex-row sm:items-center sm:justify-between">
        <span className="text-sm font-extrabold">
          x{order.totalItems} {t('totalItems', { count: order.totalItems })}
        </span>
        <span className="text-lg font-extrabold sm:text-right">
          {t('total', { amount: formatVnd(order.totalAmount) })}
        </span>
      </div>
    </article>
  );
