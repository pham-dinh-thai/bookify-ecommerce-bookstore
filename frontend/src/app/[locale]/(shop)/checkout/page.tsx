...
export default function CheckoutPage() {
  ...
  const paymentOptions: { value: PaymentMethod; label: string }[] = [
    { value: 'cash_on_delivery', label: t('cashOnDelivery') },
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (paymentMethod === 'e_wallet') {
        ...
        toast?.addToast(t('redirectingPayment'), 'success');
        window.location.href = payment.payUrl;
        return;
      }

      toast?.addToast(t('orderPlaced'), 'success');
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        {loading ? (
          ...
          <div className="rounded-lg border border-dashed border-[#3f6754]/20 bg-white p-10 text-center">
            <h2 className="text-2xl font-bold">{t('noItems')}</h2>
            ...
            <div className="space-y-6 lg:col-span-7">
              <section className="rounded-lg bg-[#eff5ef] p-5 shadow-sm sm:p-6">
                <div className="mb-6 flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold">{t('deliveryContact')}</h2>
                </div>
