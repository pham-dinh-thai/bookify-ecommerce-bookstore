    ...
    <section className="bg-[#f7faf5] px-6 pt-16 pb-20 text-[#2b352f] md:px-10 lg:px-16">
      <div className="mx-auto max-w-5xl">
        <div className="mb-16 text-center">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.4em] text-[#1b4332]">Archive Vol. IV • 2024</p>
          <h1 className="mb-5 text-5xl font-extrabold tracking-tight md:text-7xl">The Best Sellers</h1>
          <p className="mx-auto max-w-2xl text-base font-light text-[#58615b] md:text-lg">
            A definitive ranking of our top 10 literary acquisitions. Structured by intellectual depth and aesthetic permanence.
          </p>
        </div>

        {books.length === 0 ? (
          <div className="rounded-xl border border-[#aab4ad]/20 bg-white p-10 text-center">
            <p className="text-[#58615b]">No ranking data is available yet. Please check back soon.</p>
          </div>
        ) : (
          <div className="divide-y divide-[#aab4ad]/20">
            {books.map((book, index) => (
              <article
                key={book.id}
                className="group -mx-4 flex items-center gap-5 rounded-xl px-4 py-8 transition-all duration-300 hover:bg-[#1b4332]/[0.02] md:gap-10 md:py-10"
              >
                <span className="w-16 shrink-0 text-4xl font-black tabular-nums text-[#1b4332]/15 md:w-24 md:text-6xl">
                  {String(index + 1).padStart(2, '0')}
                </span>

                <div className="h-24 w-20 shrink-0 overflow-hidden rounded-sm bg-[#e8f0e9] shadow-sm transition-shadow group-hover:shadow-md md:h-32 md:w-28">
                  <img src={book.cover} alt={book.title} className="h-full w-full object-cover" />
                </div>

                <div className="min-w-0 flex-1">
                  <h2 className="truncate text-lg font-bold tracking-tight text-[#2b352f] md:text-2xl">{book.title}</h2>
                  <p className="truncate text-sm font-light italic text-[#58615b] md:text-base">{book.author}</p>
                  <p className="mt-1 text-xs uppercase tracking-[0.12em] text-[#58615b]">{book.publisher}</p>
                </div>

                <div className="shrink-0 text-right">
                  <p className="text-base font-extrabold text-[#1b4332] md:text-lg">{formatVnd(book.originalPrice)}</p>
                  <Link
                    href={`/books/${book.id}`}
                    className="mt-1 inline-block text-[11px] font-bold uppercase tracking-[0.2em] text-[#1b4332] hover:underline"
                  >
                    Add
                  </Link>
                </div>
        ...
        <div className="mt-16 text-center">
          <Link href="/books" className="inline-flex items-center gap-2 text-sm font-extrabold uppercase tracking-[0.2em] text-[#1b4332]">
            Discover Full Vault
